...
std::vector<DBLogger *> DBLogger::channels;


static void defaultCallback(const DBLogger& logger, const char * message) {
...
void DBLogger::vprintf(const char * format, va_list l, const char * file, unsigned int line) {
	char buffer[1024];
	char * cur = buffer;

	if (flags & DBLogger::FILE) cur += sprintf(cur, "%s:", file);
	if (flags & DBLogger::LINE) cur += sprintf(cur, "%d:", line);
	if (flags) cur += sprintf(cur, " ");

	//HCF
	//::vsnprintf(cur, 1024, format, l);
	//_vsnprintf(cur, 1024, format, l);
    vsnprintf(cur, 1024, format, l);

	callback(*this, buffer);
}

void DBLogger::setOutput(std::ostream * o) {
	out = o;
}

void DBLogger::setCallback(void (*cback)(const DBLogger& logger, const char * message)) {
	callback = cback;
}

void DBLogger::setFlag(unsigned int flag) {
	this->flags = flag;
}

void DBLogger::fixSize(unsigned int channel) {
	while(channel >= channels.size()) {
		channels.push_back(new DBLogger());
	}
}

std::ostream& DBLogger::getOutput() const {
	return *out;
}

void DBLogger::log(unsigned int channel, const char * file, unsigned int line, const char * format, ...) {
	fixSize(channel);
...
void DBLogger::log(unsigned int channel, const char * file, unsigned int line, std::ostream& os) {
	fixSize(channel);

	channels[channel]->setOutput(&os);
}

void DBLogger::log(unsigned int channel, const char * file, unsigned int line, unsigned int flag) {
	fixSize(channel);

	channels[channel]->setFlag(flag);
}

void DBLogger::log(unsigned int channel, const char * file, unsigned int line, void (*callback)(const DBLogger& logger, const char * message)) {
	fixSize(channel);
