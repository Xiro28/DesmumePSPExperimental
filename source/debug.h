...
class DBLogger {
protected:
	void (*callback)(const DBLogger& logger, const char * format);
	std::ostream * out;
	unsigned int flags;

	static std::vector<DBLogger *> channels;
...
public:
	...
	void setCallback(void (*cback)(const DBLogger& logger, const char * message));
	...
	static void log(unsigned int channel, const char * file, unsigned int line, void (*callback)(const DBLogger& logger, const char * message));
};

#if defined(DEBUG) || defined(GPUDEBUG) || defined(DIVDEBUG) || defined(SQRTDEBUG) || defined(DMADEBUG) || defined(DEVELOPER)
#define LOGC(channel, ...) DBLogger::log(channel, __FILE__, __LINE__, __VA_ARGS__)
#else
#define LOGC(...) {}
#endif
