...
BackupDevice::BackupDevice()
{
	...
	//HCF
	strcpy(buf, rom_filename);
	buf[strlen(rom_filename) - 4] = '\0';
...
void BackupDevice::reset()
{
	...
	if(state == DETECTING && CommonSettings.manualBackupType != MC_TYPE_AUTODETECT)
	{
		...
		ensure((u32)savesize, 0); //expand properly if necessary
...
void BackupDevice::ensure(u32 addr, int inReset, EMUFILE *fpOut)
{
	ensureReal(addr, uninitializedValue, inReset, fpOut);
}

//HCF inReset: forces a reset of the emulator after creating the save file
void BackupDevice::ensureReal(u32 addr, u8 val, int inReset, EMUFILE *fpOut)
{
