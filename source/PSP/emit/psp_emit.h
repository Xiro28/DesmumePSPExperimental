...
#define emit_insn(ins) insert_instruction(ins);
#define emit_insn2(ins,addr) insert_instruction2(ins,addr);
///////////////////////////////
...
#define emit_itypeC(_op, _rs, _rt, _imm, addr) \
	do { psp_insn_t insn; insn.itype.op = _op; insn.itype.rs = _rs; insn.itype.rt = _rt; insn.itype.imm = _imm; emit_insn2(insn,addr); } while(0)
...
#define psp_relative_target(target) psp_relative_target_2(emit_GetPtr(),target)
...
#define emit_subi(_rt, _rs, _imm) emit_imm(addi, _rs, _rt, -_imm)
...
//  | movi rt, immediate
#define emit_movi(_rt, _imm) emit_addiu(_rt, psp_zero, _imm)
...
//#define emit_vnop() emit_vtype_vd_vs_vt(0xff,0, 0, 0, 0, 0)
